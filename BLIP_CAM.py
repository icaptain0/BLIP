...
selfie_seg = mp_selfie_segmentation.SelfieSegmentation(model_selection=1)
...
def live_stream_with_caption(processor, model, device, display_width=1200, display_height=800):
    ...
    try:
        while True:
            ...
            mask = results.segmentation_mask > 0.05
            mask = mask.astype(np.uint8) * 255
            mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
            
            alpha = 0.9
            bg = (frame * (1 - alpha) + 255 * alpha).astype(np.uint8)
            person = cv2.bitwise_and(frame, mask)
            mask_inv = cv2.bitwise_not(mask)
            bg_masked = cv2.bitwise_and(bg, mask_inv)
            frame = cv2.add(person, bg_masked)
            ...
            max_width = 50  # Adjust max width for caption as needed
